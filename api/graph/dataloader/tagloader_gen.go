...
type TagLoaderConfig struct {
	...
	Fetch func(keys []string) ([]*model.Tag, []error)
...
func (l *TagLoader) Load(key string) (*model.Tag, error) {
...
func (l *TagLoader) LoadThunk(key string) func() (*model.Tag, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*model.Tag, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *TagLoader) LoadAll(keys []string) ([]*model.Tag, []error) {
	results := make([]func() (*model.Tag, error), len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *TagLoader) LoadAllThunk(keys []string) func() ([]*model.Tag, []error) {
	results := make([]func() (*model.Tag, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *TagLoader) Prime(key string, value *model.Tag) bool {
	l.mu.Lock()
	var found bool
...
func (l *TagLoader) unsafeSet(key string, value *model.Tag) {
	if l.cache == nil {
		l.cache = map[string]*model.Tag{}
	}
	l.cache[key] = value
}
